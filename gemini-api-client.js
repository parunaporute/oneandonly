...
export class GeminiApiClient {
    ...
    */
    constructor(apiKey, options = {}) {
        if (!apiKey || apiKey === 'YOUR_API_KEY') { // ダミーキーもチェック
    ...
    async generateContent(prompt, modelId) {
        ...
        if (this.#stubMode) {
            ...
            // ランダムな応答を選択
            const stubText = this.#stubResponses[Math.floor(Math.random() * this.#stubResponses.length)];
            // ダミー応答も履歴に追加
        ...
        // --- 通常モード (API呼び出し) ---
        if (!this.#apiKey) { // APIキーチェックはスタブモードでない場合のみ行う
            throw new Error('APIキーが設定されていません。');
        }
        if (!modelId) { // モデルIDチェックも同様
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const detailMessage = errorData?.error?.message || response.statusText || '不明なエラー';
                let errorMessage = `APIエラー (${response.status}): ${detailMessage}`;

                // より詳細なエラーメッセージの組み立て (元のコードから流用)
                if (response.status === 400) {
                    if (detailMessage.includes('prompt was blocked')) { errorMessage = `APIエラー(${response.status}): プロンプトが安全設定によりブロックされました。`; }
                    else if (detailMessage.includes('User location is not supported')) { errorMessage = `APIエラー(${response.status}): お住まいの地域からはこのモデルを利用できません。`; }
                    else { errorMessage = `APIエラー(${response.status}): リクエスト形式が不正です。(${detailMessage})`; }
            ...
            if (!data.candidates && data.promptFeedback?.blockReason) {
                ...
                if (data.promptFeedback.safetyRatings) {
                    generatedText += `\n(詳細: ${JSON.stringify(data.promptFeedback.safetyRatings)})`;
                }
                ...
                if (!candidate.content || !candidate.content.parts || candidate.content.parts.length === 0) {
                    generatedText = `応答が空でした。(終了理由: ${candidate.finishReason || '不明'})`;
    ...
    static async listAvailableModels(apiKey) {
        if (!apiKey || apiKey === 'YOUR_API_KEY') {
            throw new Error('APIキーが設定されていないか、有効でないためモデルリストを取得できません。');
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                try {
                    errorData = await response.json();
                } catch (e) { console.error("Failed to parse error response JSON:", e); }
                const detailMessage = errorData?.error?.message || response.statusText || '不明なエラー';
            ...
            if (data.models && data.models.length > 0) {
                ...
                const modelInfo = {
                    'gemini-1.5-pro-latest': { tier: '高性能・高機能', description: 'Googleの最も高性能な次世代モデル。複雑なタスク、長文コンテキスト処理に優れます。' },
                    'gemini-1.5-flash-latest': { tier: '高速・効率的', description: '速度とコスト効率に優れた軽量モデル。応答速度が重要なタスクや、一般的な用途に適しています。' },
                };
                ...
                data.models.forEach(model => {
                    if (model.supportedGenerationMethods?.includes('generateContent')) {
                        ...
                        if (modelId.includes('vision') || modelId.includes('embedding') || modelId.includes('aqa')) {
                            console.log(`Skipping non-text-generation or deprecated model: ${modelId}`);
                            return;
                        }
