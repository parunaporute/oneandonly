...
import {
    ...
    deleteSceneEntry,
    addSceneEntry,
    listAllSlots as dbListAllSlots,
    getUniversalSave as dbGetUniversalSave,
    putUniversalSave as dbPutUniversalSave,
    ...
    try {
        ...
        allSlots.forEach((slot) => {
            ...
            if (!slot.data) {
                ...
                label.innerHTML = `<b>${slot.slotIndex}:</b> ${DOMPurify.sanitize(
                    title
                )} <small style="color:#bbb;">(${scenes}シーン, ${date})</small>`;
            }
            row.appendChild(rb);
...
export async function onDeleteSlot(slotIndex) {
    ...
    const confirmationMessage = isDefaultSlot
        ? `<p>スロット ${slotIndex} のデータをクリアして空きスロットに戻しますか？<br>(スロット番号 ${slotIndex} は残ります)</p>`
        : `<p style="color: #ffcccc;"><strong>警告:</strong> スロット ${slotIndex} を完全に削除します。<br>このスロット番号は再利用されにくくなります。よろしいですか？</p>`;
...
export async function onClickLoad(/* modalInstance */) {
    ...
    try {
        ...
        if (currentModal) currentModal.close();
            ...
            console.warn('[SaveLoad] Could not find modal instance to close before load/redirect.');
        ...
        if (window.currentScenarioId && targetScenarioId === window.currentScenarioId) {
            ...
            multiModalOpen({
                ...
                onOk: async () => {
                    ...
                    const confirmModal =
                        window.multiModal?.getInstanceById?.('data-overwrite-confirm'); // 仮ID
    ...
    try {
        ...
        for (const sc of scenes) {
            const rec = {
                ...
                sceneId:
                    sc.sceneId ||
                    `scene_${Date.now()}_${Math.random().toString(16).substring(2, 8)}`,
                content: sc.content || '',
            ...
            addProms.push(
                addSceneEntry(rec).then(async (newId) => {
                    ...
                    if (
                        rec.content &&
                        typeof window.generateImagePromptFromScene === 'function' &&
                        window.geminiClient
                    ) {
                        try {
